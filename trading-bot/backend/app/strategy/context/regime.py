...
class RegimeDetector:
    def identify(self, latest_bars: pd.DataFrame) -> RegimeResult:
        ...
        if volatility_ratio > 1.8: regime = MarketRegime.RANGING_EXPANDED
            ...
            if slope > (curr_atr * 0.1): regime = MarketRegime.TRENDING_BULLISH
            elif slope < -(curr_atr * 0.1): regime = MarketRegime.TRENDING_BEARISH
            else: regime = MarketRegime.RANGING_EXPANDED
            
        return RegimeResult(regime=regime, confidence=0.85, 
                            metrics={"vol_ratio": volatility_ratio, "slope": slope})
