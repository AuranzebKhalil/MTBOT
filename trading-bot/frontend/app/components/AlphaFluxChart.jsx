...
export default function AlphaFluxChart({ symbol }) {
  ...
  useEffect(() => {
    ...
    if (settings.showBOS && overlays.bos_markers) {
       overlays.bos_markers.forEach(m => rawMarkers.push({ time: m.time, position: m.type === "bearish" ? "aboveBar" : "belowBar", color: m.type === "bearish" ? "#ff453a" : "#32d74b", shape: m.type === "bearish" ? "arrowDown" : "arrowUp", text: "BOS" }));
    }
    if (settings.showSweeps && overlays.sweeps) {
       overlays.sweeps.forEach(s => rawMarkers.push({ time: s.time, position: s.type === "bullish" ? "belowBar" : "aboveBar", color: "#bf5af2", shape: "circle", text: "LIQ" }));
    }
    if (settings.showMSS && overlays.mss_markers) {
       overlays.mss_markers.forEach(m => rawMarkers.push({ time: m.time, position: m.type === "bearish" ? "aboveBar" : "belowBar", color: "#ffd60a", shape: "circle", text: "MSS" }));
    }
    
    // Sort and limit markers to avoid clutter
    rawMarkers.sort((a, b) => a.time - b.time);
    const limitedMarkers = rawMarkers.slice(-50); // Only show last 50 markers to keep chart clean
    
    series.setMarkers(limitedMarkers);

    if (settings.showOB && overlays.order_blocks) {
       overlays.order_blocks.slice(-3).forEach(ob => {
         const primitive = new ZonePrimitive({ time1: ob.time, price1: ob.top, price2: ob.bottom, color: ob.type === "bullish" ? "rgba(50, 215, 75, 0.08)" : "rgba(255, 69, 58, 0.08)", border: true, label: "OB" }, theme);
    ...
    if (settings.showFVG && overlays.fvg_zones) {
      overlays.fvg_zones.slice(-3).forEach(f => {
        const primitive = new ZonePrimitive({ time1: f.time, price1: f.top, price2: f.bottom, color: "rgba(34, 163, 240, 0.04)", border: true, label: "FVG" }, theme);
        series.attachPrimitive(primitive);
        overlaysRef.current.push({ primitive });
      });
    }
    ...
    <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ 
        ...
        boxShadow: "0 4px 20px rgba(0,0,0,0.1)"
         ...
         <LegendItem color={theme === 'dark' ? "rgba(34, 163, 240, 0.6)" : "rgba(34, 163, 240, 0.8)"} label="FVG GAP" />
